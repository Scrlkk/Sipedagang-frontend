...
const emit = defineEmits([
  ...
  'update:currentPage',  'update:pageInput',
  'page-input-submit'
])
...
<template>  <div class="w-full max-w-7xl mx-auto px-2 sm:px-4 md:px-6">
    ...
    <div class="bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden flex flex-col h-auto max-h-[95vh] sm:max-h-[90vh] md:max-h-[80vh] border border-gray-100">      <!-- Header with blue background and improved styling -->
          ...
          <div class="w-full sm:w-auto">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div class="relative w-full group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none group-focus-within:text-blue-600 transition-colors duration-200">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 group-focus-within:text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                >
                  <div class="bg-gray-100 hover:bg-red-100 rounded-full p-1 transition-colors duration-200">
              ...
              <div class="relative w-full group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none group-focus-within:text-indigo-600 transition-colors duration-200">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 group-focus-within:text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                  <div class="bg-indigo-100 rounded-full p-1 text-indigo-600 transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      </div>      <!-- Modern table design with glass-morphism style and enhanced scrolling -->
      <div class="overflow-y-auto overflow-x-auto scrollbar-fancy px-1 sm:px-2 py-1 table-container" style="max-height: calc(100vh - 250px); min-height: 160px;">
          ...
          <div class="overflow-hidden border border-gray-200/50 sm:rounded-xl shadow-md bg-white/80 backdrop-blur-sm sm:shadow-lg rounded-lg">
            ...
            <table class="min-w-full divide-y divide-gray-200/50 table-auto sm:table-fixed">              <thead class="bg-[#0099FF]/10 sticky top-0 z-10 shadow-sm">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <div class="tooltip py-1 font-medium">
                      ...
                      <div class="block sm:hidden">
                        <div class="flex flex-col">
                          <span class="font-semibold text-gray-900">{{ item.jenisPengadaan }}</span>
                      ...
                      <!-- Desktop layout -->
                      <div class="hidden sm:block">{{ item.jenisPengadaan }}</div>
                      <span class="tooltiptext">{{ item.jenisPengadaan }}</span>
                    </div>
                  </td><td class="hidden xs:table-cell px-3 py-4 sm:px-4 text-sm text-gray-700 truncate max-w-[100px] sm:max-w-none border-b border-gray-200/30">
                    <div class="tooltip py-1 font-medium text-indigo-800">
                      {{ item.noProorder }}
                      <span class="tooltiptext">{{ item.noProorder }}</span>
                    </div>
                  </td>
                  <td class="hidden sm:table-cell px-3 py-4 sm:px-4 text-sm text-gray-700 truncate max-w-[100px] sm:max-w-none border-b border-gray-200/30">
                    <div class="tooltip py-1">
                      {{ item.supplier }}
                      <span class="tooltiptext">{{ item.supplier }}</span>
                    </div>
                  </td>
                  <td class="hidden md:table-cell px-3 py-4 sm:px-4 text-sm text-gray-700 truncate max-w-[100px] sm:max-w-none border-b border-gray-200/30">
                    <div class="tooltip py-1">
                      {{ item.perusahaan }}
                      <span class="tooltiptext">{{ item.perusahaan }}</span>
                    </div>
                  </td>
                  <td class="hidden xs:table-cell px-3 py-4 sm:px-4 text-sm text-gray-700 truncate max-w-[80px] sm:max-w-none border-b border-gray-200/30">
                    <div class="tooltip py-1 font-medium text-gray-800">
                      {{ item.kuantum }}
                      <span class="tooltiptext">{{ item.kuantum }}</span>
                    </div>
                  </td>                  <td class="px-3 py-4 sm:px-4 text-sm text-gray-700 truncate max-w-[60px] sm:max-w-none border-b border-gray-200/30">
                    ...
                    <div class="hidden sm:flex items-center justify-center">
                      <div class="inline-flex bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-gray-100/80 p-0.5">                        
                        <router-link 
                          ...
                          title="Preview"
                        >
                          <div class="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50 shadow-lg">
                    ...
                    <div class="bg-white/80 backdrop-blur-sm rounded-xl p-3 border border-gray-100">
                      ...
                      <div class="flex flex-col mb-3">
                        ...
                        <div class="flex items-center justify-between mb-2">
                          <h3 class="text-sm font-bold text-gray-900 truncate max-w-[75%]">{{ item.jenisPengadaan }}</h3>
                          <span class="inline-block bg-indigo-100 text-indigo-800 text-xs font-semibold px-2 py-1 rounded-md">
                            {{ item.tanggal }}
                          </span>
                        </div>
                        ...
                        <div class="flex items-center mb-1 bg-[#0099FF]/10 p-1.5 rounded-lg">
                          ...
                          <div>
                            <span class="text-xs font-medium text-indigo-900 block leading-tight">No Preorder:</span>
                            <span class="text-sm font-semibold text-gray-900">{{ item.noProorder || 'N/A' }}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="grid grid-cols-1 gap-2.5 mb-3">
                        ...
                        <div class="grid grid-cols-2 gap-2.5">
                          <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-2 rounded-lg">
                            <div class="flex items-center mb-1">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-blue-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                              </svg>
                              <span class="text-xs font-semibold text-indigo-700">Perusahaan:</span>
                            </div>
                            <span class="text-sm text-gray-800 line-clamp-2">{{ item.perusahaan || 'N/A' }}</span>
                          </div>
                          
                          <div class="bg-gradient-to-r from-purple-50 to-blue-50 p-2 rounded-lg">
                            <div class="flex items-center mb-1">
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-purple-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                              </svg>
                              <span class="text-xs font-semibold text-indigo-700">Supplier:</span>
                            </div>
                            <span class="text-sm text-gray-800 line-clamp-2">{{ item.supplier || 'N/A' }}</span>
                          </div>
                        </div>
                        
                        <!-- Quantity info with icon -->
                        <div class="bg-gradient-to-r from-green-50 to-teal-50 p-2 rounded-lg">
                          <div class="flex items-center mb-1">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" />
                            </svg>
                            <span class="text-xs font-semibold text-green-700">Kuantum:</span>
                          </div>
                          <span class="text-sm font-medium text-gray-800 block py-1">{{ item.kuantum || 'N/A' }}</span>
                        </div>
                      </div>
                ...
                </tr><!-- Empty rows to maintain consistent height when data is sparse -->
                ...
                </template>                <!-- Beautiful empty state with modern design -->
                <tr v-if="paginatedData.length === 0" class="empty-state-row">
                  <td colspan="7" class="px-4 py-0 text-center align-middle empty-state-cell">
                    <div class="flex items-center justify-center max-w-sm mx-auto bg-white/90 p-3 px-4 rounded-xl shadow-sm border border-gray-100/80">
                      <div class="mr-3 p-2 bg-gradient-to-r from-blue-100/50 to-indigo-100/50 rounded-full border-2 border-white shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                        </svg>
                      </div>
                      <div class="flex-1">
                        <h3 class="text-base text-left font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-0.5">
                          Tidak ada data
                        </h3>
                        <p class="text-sm text-left text-gray-500 mb-0">
                          {{ searchQuery || selectedMonth ? 'Tidak ada data yang sesuai dengan filter yang dipilih' : 'Belum ada data yang tersedia' }}
                        </p>
                      </div>
      ...
      <div class="p-4 sm:p-5 border-t border-gray-100 bg-gradient-to-b from-white to-gray-50/80 flex flex-col sm:flex-row justify-between items-center rounded-b-xl">
        <!-- Enhanced pagination with glass morphism styling -->
        <div class="flex items-center mb-4 sm:mb-0 overflow-x-auto scrollbar-fancy pb-2 max-w-full">
          <!-- Beautiful mobile pagination display with gradient -->
          <div class="block sm:hidden text-center bg-gradient-to-r from-blue-50 to-indigo-50 py-1 px-3 border border-indigo-100 rounded-lg shadow-sm mr-2">
          ...
          <div class="flex items-center space-x-2 min-w-max">
            ...
            <div 
              ...
              :class="[
                ...
                (page !== 1 && page !== getTotalPages && page !== currentPage && window.innerWidth < 380)
                  ? 'hidden xs:flex' : ''
              ]"
            >
              {{ page }}
            ...
            <div class="relative ml-3 hidden sm:flex items-center">
              <span class="mr-2 text-gray-700 text-sm whitespace-nowrap font-medium">Halaman:</span>
              <div class="flex items-center bg-white/70 backdrop-blur-sm border border-gray-100 rounded-lg shadow-sm px-1.5">                <input
                  type="number"
                  :value="pageInput"
                  @input="$emit('update:pageInput', Number($event.target.value))"
                  @keyup.enter="$emit('update:pageInput', Number($event.target.value)); $emit('page-input-submit')"
                  @blur="$emit('update:pageInput', Number($event.target.value)); $emit('page-input-submit')"
                  min="1"
                  :max="getTotalPages"
                ...
                />
                <span class="mx-1 text-gray-500">/ <span class="text-gray-700">{{ getTotalPages }}</span></span>
              </div>
            </div>
          </div>
        </div>
