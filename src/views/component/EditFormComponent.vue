  ...
  <div class="w-full max-w-5xl mx-auto px-2 sm:px-4">
    <div class="bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col h-auto max-h-[82vh] sm:max-h-[80vh] border border-gray-100 form-card">
      ...
      <div class="text-center pb-2 bg-white z-10 flex-shrink-0">
        <h1 class="text-lg sm:text-xl font-medium mt-4 sm:mt-6 mb-1 form-title" style="color: #0099FF;">Edit Form Input Data</h1>
      ...
      <div class="overflow-y-auto flex-grow min-h-0" role="main" aria-label="Edit form data permohonan">
        <div class="p-3 sm:p-6 pb-2 sm:pb-6">
          ...
          <fieldset class="mb-4 sm:mb-6 form-section">
            ...
            <!-- Apply the same accessibility improvements as FormComponent -->
            <div class="space-y-3">
              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-namaSupplier" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                  Nama Supplier <span class="text-red-500" aria-label="wajib diisi">*</span>
                </label>
                <input
                  id="edit-namaSupplier"
                  type="text"
                  :value="namaSupplier"
                  ...
                  required
                  aria-describedby="edit-namaSupplier-help"
                />
                <div id="edit-namaSupplier-help" class="sr-only">Masukkan nama supplier yang mengajukan permohonan</div>
              </div>

              <!-- Continue with all other fields using similar accessibility patterns... -->
              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-namaPerusahaan" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                ...
                <input
                  ...
                  required
                  aria-describedby="edit-namaPerusahaan-help"
                />
                <div id="edit-namaPerusahaan-help" class="sr-only">Masukkan nama perusahaan supplier</div>
              </div>

              <!-- Continue with the rest of the fields following the same pattern... -->
              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-jenisBank" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                  Jenis Bank
                </label>
                <div class="relative w-full sm:w-3/4">
                  <select
                    ...
                    aria-describedby="edit-jenisBank-help"
                  >
                ...
                <div id="edit-jenisBank-help" class="sr-only">Pilih jenis bank untuk rekening supplier</div>
              </div>

              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-nomerRekening" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                  Nomer Rekening <span class="text-red-500" aria-label="wajib diisi">*</span>
                </label>
                <input
                  id="edit-nomerRekening"
                  type="text"
                  :value="nomerRekening"
                  ...
                  required
                  aria-describedby="edit-nomerRekening-help"
                />
                <div id="edit-nomerRekening-help" class="sr-only">Masukkan nomor rekening bank supplier</div>
              </div>
            </div>
          </fieldset>
          ...
          <fieldset class="mb-4 sm:mb-6 form-section">
            ...
            <!-- Apply similar accessibility patterns to all fields... -->
          </fieldset>
            ...
            <div class="space-y-3" role="group" aria-label="Daftar data IN">
              <div v-for="(item, index) in dataIN" :key="index" class="flex items-center space-x-2 p-2 rounded">
                ...
                <div class="flex-none w-32 sm:w-44 lg:w-56">
                  <label :for="`edit-tanggal-${index}`" class="sr-only">Tanggal untuk baris {{ index + 1 }}</label>
                  <input
                    :id="`edit-tanggal-${index}`"
                    type="date"
                    :value="item.tanggal"
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex">
                    <label :for="`edit-kuantum-${index}`" class="sr-only">Kuantum untuk baris {{ index + 1 }}</label>
                    <input
                      :id="`edit-kuantum-${index}`"
                      type="text"
                      :value="item.kuantum"
                    ...
                    <select
                      ...
                      class="px-2 sm:px-3 py-2 border border-l-0 border-gray-300 rounded-r-md bg-white text-gray-500 focus:outline-none focus:ring-2 focus:ring-[#0099FF] focus:border-transparent text-xs sm:text-sm"
                      :aria-label="`Satuan untuk baris ${index + 1}`"
                    >
          ...
          <fieldset class="mb-4 sm:mb-6 form-section">
            ...
            <div class="space-y-3">
              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-jumlahPembayaran" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                  Jumlah Pembayaran
                </label>
                <input
                  id="edit-jumlahPembayaran"
                  type="text"
                  ...
                  aria-describedby="edit-jumlahPembayaran-help"
                />
                <div id="edit-jumlahPembayaran-help" class="sr-only">Masukkan jumlah pembayaran dalam rupiah</div>
              </div>
              
              <div class="flex flex-col sm:flex-row sm:items-center">
                <label for="edit-jumlahSPP" class="block text-gray-700 sm:w-1/4 mb-1 sm:mb-0 text-sm sm:text-base font-medium">
                  Jumlah di-SPP
                </label>
                <input
                  id="edit-jumlahSPP"
                  ...
                  aria-describedby="edit-jumlahSPP-help"
                />
                <div id="edit-jumlahSPP-help" class="sr-only">Masukkan jumlah dalam SPP (Surat Permintaan Pembayaran)</div>
              </div>
            </div>
          </fieldset>
        </div>
      ...
      <div class="flex-shrink-0 px-3 sm:px-6 py-3 sm:py-4 bg-white border-t border-gray-100 z-10 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4">
        ...
        <button 
          ...
          aria-label="Simpan perubahan data form"
        >
          Save
        </button>
      </div>
