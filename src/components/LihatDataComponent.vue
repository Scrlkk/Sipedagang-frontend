...
<template>
  ...
  <div class="p-4 max-w-5xl mx-auto">
    <!-- Simple Content Card -->
    <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">      <!-- Simple Header -->
      <div class="bg-blue-600 p-4">
        ...
        <div class="block sm:hidden mb-3">
          <router-link
            to="/admin/dashboard"
            @click.native="goBack"
            class="inline-flex items-center px-3 py-1.5 bg-white/20 text-white text-sm font-medium rounded border border-white/30 hover:bg-white/30"
          >
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start gap-4">
          ...
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <p class="text-white/80 text-sm">Data Pengadaan</p>
            </div>
          </div>
          ...
          <div class="w-full sm:w-auto flex flex-col sm:flex-row gap-2">
            ...
            <div class="relative">
              ...
              />
              <svg class="absolute left-3 top-2.5 h-4 w-4 text-white/60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="relative">
              ...
              <svg class="absolute right-3 top-2.5 h-4 w-4 text-white/60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Active Filters -->
        <div v-if="searchQuery || selectedMonth" class="flex flex-wrap gap-2 mt-3">
          <span v-if="searchQuery" class="inline-flex items-center px-3 py-1 bg-white/20 text-white text-xs rounded-full">
            Pencarian: {{ searchQuery }}
            <button @click="$emit('update:searchQuery', '')" class="ml-2 text-white/80 hover:text-white">
              <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </span>
          <span v-if="selectedMonth" class="inline-flex items-center px-3 py-1 bg-white/20 text-white text-xs rounded-full">
            Bulan: {{ availableMonths.find(m => m.value === selectedMonth)?.label }}
            <button @click="$emit('update:selectedMonth', '')" class="ml-2 text-white/80 hover:text-white">
              <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </span>
          <span class="px-3 py-1 bg-white/30 text-white text-xs rounded-full font-medium">
        ...
        <div class="block md:hidden p-4 space-y-3">
          ...
          >
            ...
            <div class="flex justify-between items-start mb-3">
              <div>
                <h3 class="font-semibold text-gray-900 text-sm">{{ getItemValue(item, 'jenis_pengadaan') }}</h3>
                <p class="text-xs text-gray-500">{{ getItemValue(item, 'no_preorder') }}</p>
              </div>
              <div class="flex space-x-1">
                <button @click="printItem(item)" class="p-1.5 bg-purple-100 text-purple-600 rounded hover:bg-purple-200">
                  <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                  </svg>
                </button>
                <button @click="editItem(item)" class="p-1.5 bg-green-100 text-green-600 rounded hover:bg-green-200">
                  <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button @click="deleteItem(item)" class="p-1.5 bg-red-100 text-red-600 rounded hover:bg-red-200">
                  <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <div class="font-medium text-gray-900">{{ getItemValue(item, 'supplier') }}</div>
              </div>
              <div>
                <span class="text-gray-500">Perusahaan:</span>
                <div class="font-medium text-gray-900">{{ getItemValue(item, 'perusahaan') }}</div>
              </div>
              <div>
                <span class="text-gray-500">Kuantum:</span>
                <div class="font-medium text-blue-600">{{ getItemValue(item, 'kuantum') }}</div>
              </div>
              <div>
                <span class="text-gray-500">Tanggal:</span>
                <div class="font-medium text-gray-900">{{ getItemValue(item, 'tanggal') }}</div>
              </div>
            </div>
          </div>

          <!-- Empty State Mobile -->
          <div v-if="computedPaginatedData.length === 0" class="text-center py-8">
            ...
            <p class="text-gray-400 text-sm">Data yang dicari tidak ditemukan</p>
          </div>
        </div>
        ...
        <div class="hidden md:block">
          <table class="w-full">
            ...
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Jenis Pengadaan</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">No. Preorder</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Supplier</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Perusahaan</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Kuantum</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                <td class="px-4 py-3">
                  <div class="text-sm font-medium text-gray-900">{{ getItemValue(item, 'jenis_pengadaan') }}</div>
                </td>
                <td class="px-4 py-3">
                  <div class="text-sm text-gray-900">{{ getItemValue(item, 'no_preorder') }}</div>
                </td>
                <td class="px-4 py-3">
                  <div class="text-sm text-gray-900">{{ getItemValue(item, 'supplier') }}</div>
                </td>
                <td class="px-4 py-3">
                  <div class="text-sm text-gray-900">{{ getItemValue(item, 'perusahaan') }}</div>
                </td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3">
                  <div class="text-sm text-gray-900">{{ getItemValue(item, 'tanggal') }}</div>
                </td>
                <td class="px-4 py-3">
      ...
      </div>      <!-- Simple Footer with Pagination -->
      <div class="bg-gray-50 border-t border-gray-200 p-4">
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
          <!-- Pagination Info -->
          <div class="text-sm text-gray-600">
            Menampilkan {{ (currentPage - 1) * itemsPerPage + 1 }} - {{ Math.min(currentPage * itemsPerPage, computedFilteredData.length) }} dari {{ computedFilteredData.length }} data
          </div>
          ...
          <div class="flex items-center space-x-2">
            ...
            <button
              ...
              :class="currentPage === 1 ? 'text-gray-400 bg-gray-100 cursor-not-allowed border-gray-200' : 'text-gray-700 bg-white hover:bg-gray-50 border-gray-300'"
            >
              ‹ Prev
            </button>
            ...
            <button
              ...
              class="px-3 py-1 border rounded text-sm"
              :class="currentPage === computedTotalPages ? 'text-gray-400 bg-gray-100 cursor-not-allowed border-gray-200' : 'text-gray-700 bg-white hover:bg-gray-50 border-gray-300'"
            >
              Next ›
            </button>
          </div>          <!-- Back Button (Desktop only) -->
